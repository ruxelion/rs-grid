---
title: Rows
description: Row records, row height, row number gutter, hover, and alternating backgrounds.
---
...
Row indices are `u64` (not `usize`), supporting up to ~9×10¹⁴ rows
with full f64 precision. On WASM32, `usize` is only 32 bits (4 GB),
which is why row indices use `u64` throughout.

<Warning>
  Beyond ~9×10¹⁴ rows, f64 precision degrades (consecutive integers
