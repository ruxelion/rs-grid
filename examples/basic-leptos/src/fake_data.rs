...
fn hash_field(row: u64, field: u64) -> u64 {
    hash_row(
        row.wrapping_add(
            field.wrapping_mul(0x517cc1b727220a95),
        ),
    )
}
...
const FIRST_NAMES: &[&str] = &[
    ...
    "Kenji", "Laura", "Miguel", "Nadia", "Oliver",
    "Priya", "Quentin", "Rachel", "Sanjay", "Tara",
    "Uma", "Victor", "Wendy", "Xavier", "Yuki",
    "Zara", "Aaron", "Beatrice", "Carlos", "Delilah",
    "Ethan", "Fiona", "Gabriel", "Helena", "Isaac",
    "Jasmine", "Kevin", "Lily", "Marco", "Nina",
    "Oscar", "Penelope", "Rafael", "Sofia", "Thomas",
    "Ursula", "Vincent", "Whitney", "Xander", "Yvette",
    "Zachary", "Amelia", "Benjamin", "Clara", "David",
    "Elena", "Felix", "Grace", "Henry", "Iris",
    "James", "Katherine", "Leo", "Maya", "Nathan",
    "Olivia", "Patrick", "Quinn", "Rosa", "Samuel",
    "Tiffany", "Ulrich", "Valentina", "Walter",
    "Ximena", "Yolanda", "Zane", "Aria", "Brandon",
    "Chloe", "Daniel", "Emma", "Frank", "Giselle",
    "Hugo", "Ingrid", "Joel", "Kira", "Lucas", "Mia",
    "Noah", "Paige", "Remy", "Stella", "Tyler",
    "Violet", "Wesley", "Yara", "Aiden", "Bianca",
    "Caleb", "Daphne",
];
...
const LAST_NAMES: &[&str] = &[
    ...
    "Thompson", "White", "Lopez", "Lee", "Harris",
    ...
    "Gutierrez", "Ortiz", "Morgan", "Cooper", "Chen",
    ...
    "Bell", "Coleman", "Butler", "Tanaka", "Muller",
    "Fischer", "Weber", "Schneider", "Johansson",
    "Larsson", "Dubois", "Laurent", "Bernard", "Rossi",
    "Bianchi", "Colombo", "Ivanov", "Petrov",
    "Nakamura", "Yamamoto", "Sato", "Takahashi",
];
...
const CITIES: &[&str] = &[
    ...
    "Sydney", "Toronto", "Mumbai", "Singapore", "Seoul",
    "Amsterdam", "Dubai", "Stockholm", "Barcelona",
    "Chicago", "San Francisco", "Austin", "Denver",
    "Seattle", "Portland",
];
...
const STATES_US: &[&str] = &[
    "CA", "NY", "TX", "WA", "IL", "FL", "MA", "CO",
    "OR", "GA", "NC", "PA", "OH", "AZ", "VA",
];
...
const OFFICES: &[&str] = &[
    "HQ", "West Campus", "East Tower", "South Wing",
    "Remote", "Satellite", "Downtown", "Tech Park",
];
...
const SKILLS: &[&str] = &[
    ...
    "C++", "React", "Vue", "Angular", "Node.js",
    "PostgreSQL", "Redis", "Docker", "Kubernetes",
    "AWS", "GCP", "Azure", "GraphQL", "REST", "gRPC",
];

const CERTS: &[&str] = &[
    "AWS Solutions Architect", "PMP", "Scrum Master",
    "CPA", "CISSP", "Google Cloud Pro",
    "Six Sigma Green Belt", "ITIL v4", "PHR", "CFA",
];

const SPOKEN_LANGS: &[&str] = &[
    "English", "Spanish", "French", "German",
    "Japanese", "Mandarin", "Portuguese", "Korean",
    "Hindi", "Arabic",
];
...
const UNIVERSITIES: &[&str] = &[
    ...
    "Cambridge", "ETH Zurich", "UC Berkeley",
    "Caltech", "Princeton", "Yale", "Columbia",
    "Carnegie Mellon", "Georgia Tech", "U Michigan",
    "UCLA",
];
...
const LAPTOPS: &[&str] = &[
    "MacBook Pro 16", "MacBook Pro 14", "ThinkPad X1",
    "Dell XPS 15", "Surface Pro", "Framework 16",
];
...
const TIMEZONES: &[&str] = &[
    "UTC-8", "UTC-5", "UTC+0", "UTC+1", "UTC+2",
    "UTC+5:30", "UTC+8", "UTC+9",
];

const PREV_COMPANIES: &[&str] = &[
    "Google", "Amazon", "Microsoft", "Apple", "Meta",
    "Netflix", "Uber", "Stripe", "Shopify",
    "Salesforce", "Oracle", "IBM", "Intel", "Cisco",
    "Adobe",
];

const REFERRAL_SOURCES: &[&str] = &[
    "LinkedIn", "Employee Referral", "Job Board",
    "Recruiter", "University", "Direct Application",
];
...
const CLEARANCES: &[&str] =
    &["None", "Confidential", "Secret", "Top Secret"];

const STREETS: &[&str] = &[
    "Oak St", "Main St", "Elm Ave", "Park Blvd",
    "Cedar Ln", "Maple Dr", "Pine Rd", "Walnut St",
    "Broadway", "1st Ave", "2nd Ave", "3rd Ave",
];
...
pub static EXTRA_COLUMNS: &[ExtraColDef] = &[
    ...
    ec("birth_date", "Birth Date", 110.0, T,
        Gen::Date(1960, 2000)),
    ...
    ec("state", "State", 60.0, T,
        Gen::Pick(STATES_US)),
    ec("country", "Country", 160.0, IT,
        Gen::Country),
    // ── Work ────────────────────────────────────
    ec("manager", "Manager", 180.0, T, Gen::Profile),
    ec("team", "Team", 100.0, T, Gen::Pick(TEAMS)),
    ec("office", "Office", 120.0, T,
        Gen::Pick(OFFICES)),
    ec("floor", "Floor", 60.0, I, Gen::Int(1, 30)),
    ec("badge_id", "Badge ID", 90.0, I,
        Gen::Int(10_000, 99_999)),
    ec("emp_type", "Type", 100.0, T,
        Gen::Pick(EMP_TYPES)),
    ec("seniority", "Seniority", 100.0, T,
        Gen::Pick(SENIORITY_LEVELS)),
    ec("experience", "Exp. (yrs)", 80.0, I,
        Gen::Int(0, 35)),
    // ── Contact ─────────────────────────────────
    ec("mobile", "Mobile", 150.0, T, Gen::Phone),
    ec("extension", "Ext.", 70.0, I,
        Gen::Int(1000, 9999)),
    ec("slack", "Slack", 150.0, T, Gen::Handle),
    ec("linkedin", "LinkedIn", 180.0, T,
        Gen::Profile),
    // ── Performance ─────────────────────────────
    ec("rating", "Rating", 70.0, I, Gen::Int(1, 5)),
    ec("last_review", "Last Review", 110.0, T,
        Gen::Date(2024, 2026)),
    ec("goals_met", "Goals Met", 80.0, P,
        Gen::Int(40, 100)),
    ec("projects_done", "Projects", 80.0, I,
        Gen::Int(0, 25)),
    ec("peer_score", "Peer Score", 90.0, I,
        Gen::Int(60, 100)),
    ec("promo_eligible", "Promo", 70.0, B,
        Gen::Bool(30)),
    // ── Financial ───────────────────────────────
    ec("bonus", "Bonus", 100.0, C,
        Gen::Int(0, 50_000)),
    ec("stock_opts", "Stock Opts", 100.0, I,
        Gen::Int(0, 10_000)),
    ec("benefits", "Benefits", 100.0, T,
        Gen::Pick(BENEFITS)),
    ec("tax_bracket", "Tax %", 70.0, P,
        Gen::Int(15, 37)),
    ec("commission", "Commission", 100.0, C,
        Gen::Int(0, 40_000)),
    ec("total_comp", "Total Comp", 120.0, C,
        Gen::Int(50_000, 400_000)),
    // ── Skills ──────────────────────────────────
    ec("skill1", "Primary Skill", 130.0, T,
        Gen::Pick(SKILLS)),
    ec("skill2", "Secondary Skill", 130.0, T,
        Gen::Pick(SKILLS)),
    ec("cert", "Certification", 170.0, T,
        Gen::Pick(CERTS)),
    ec("language", "Language", 100.0, T,
        Gen::Pick(SPOKEN_LANGS)),
    ec("education", "Education", 110.0, T,
        Gen::Pick(EDU_LEVELS)),
    ec("university", "University", 150.0, T,
        Gen::Pick(UNIVERSITIES)),
    // ── Time ────────────────────────────────────
    ec("hours_week", "Hrs/Wk", 70.0, I,
        Gen::Int(20, 60)),
    ec("vacation_days", "Vacation", 70.0, I,
        Gen::Int(0, 30)),
    ec("sick_days", "Sick Days", 70.0, I,
        Gen::Int(0, 12)),
    ec("overtime", "Overtime", 70.0, I,
        Gen::Int(0, 20)),
    ec("remote_days", "Remote/Wk", 70.0, I,
        Gen::Int(0, 5)),
    ec("timezone", "Timezone", 90.0, T,
        Gen::Pick(TIMEZONES)),
    // ── IT ──────────────────────────────────────
    ec("laptop", "Laptop", 140.0, T,
        Gen::Pick(LAPTOPS)),
    ec("os", "OS", 90.0, T, Gen::Pick(OSES)),
    ec("vpn_on", "VPN", 60.0, B, Gen::Bool(70)),
    ec("last_login", "Last Login", 110.0, T,
        Gen::Date(2026, 2026)),
    ec("disk_gb", "Disk (GB)", 80.0, I,
        Gen::Int(128, 2048)),
    ec("tickets_open", "Open Tix", 70.0, I,
        Gen::Int(0, 15)),
    // ── Metrics ─────────────────────────────────
    ec("cust_sat", "CSAT", 60.0, I,
        Gen::Int(1, 10)),
    ec("deals_closed", "Deals", 70.0, I,
        Gen::Int(0, 50)),
    ec("tix_resolved", "Resolved", 70.0, I,
        Gen::Int(0, 200)),
    ec("nps", "NPS", 60.0, I, Gen::Int(-100, 100)),
    ec("resp_time_ms", "Resp (ms)", 80.0, I,
        Gen::Int(50, 5000)),
    ec("uptime_pct", "Uptime %", 70.0, P,
        Gen::Int(95, 100)),
    ec("velocity", "Velocity", 70.0, I,
        Gen::Int(5, 40)),
    ec("bugs_fixed", "Bugs Fixed", 80.0, I,
        Gen::Int(0, 100)),
    // ── Background ──────────────────────────────
    ec("prev_company", "Prev. Company", 140.0, T,
        Gen::Pick(PREV_COMPANIES)),
    ec("referral", "Referral", 130.0, T,
        Gen::Pick(REFERRAL_SOURCES)),
    ec("start_salary", "Start Salary", 110.0, C,
        Gen::Int(35_000, 180_000)),
    ec("relocations", "Relocations", 70.0, I,
        Gen::Int(0, 5)),
    ec("awards", "Awards", 70.0, I,
        Gen::Int(0, 8)),
    ec("publications", "Pubs", 60.0, I,
        Gen::Int(0, 20)),
    ec("patents", "Patents", 60.0, I,
        Gen::Int(0, 5)),
    ec("mentees", "Mentees", 70.0, I,
        Gen::Int(0, 10)),
    // ── Sales / Revenue ─────────────────────────
    ec("q_revenue", "Q Revenue", 110.0, C,
        Gen::Int(0, 500_000)),
    ec("pipeline", "Pipeline", 110.0, C,
        Gen::Int(0, 1_000_000)),
    ec("win_rate", "Win Rate", 70.0, P,
        Gen::Int(10, 80)),
    ec("avg_deal", "Avg Deal", 100.0, C,
        Gen::Int(5_000, 200_000)),
    ec("churn_pct", "Churn %", 70.0, P,
        Gen::Int(1, 25)),
    ec("ltv", "LTV", 100.0, C,
        Gen::Int(10_000, 500_000)),
    ec("arr", "ARR", 100.0, C,
        Gen::Int(50_000, 2_000_000)),
    ec("mrr", "MRR", 100.0, C,
        Gen::Int(4_000, 170_000)),
    // ── Misc / Preferences ──────────────────────
    ec("fav_lang", "Fav Language", 120.0, T,
        Gen::Pick(PROG_LANGS)),
    ec("ide", "IDE", 100.0, T, Gen::Pick(IDES)),
    ec("monitors", "Monitors", 70.0, I,
        Gen::Int(1, 4)),
    ec("standing", "Standing Desk", 70.0, B,
        Gen::Bool(45)),
    ec("coffee", "Coffee/Day", 70.0, I,
        Gen::Int(0, 8)),
    ec("meeting_hrs", "Mtg Hrs/Wk", 80.0, I,
        Gen::Int(2, 30)),
    ec("code_reviews", "Reviews/Wk", 80.0, I,
        Gen::Int(0, 20)),
    ec("deploys", "Deploys/Wk", 80.0, I,
        Gen::Int(0, 15)),
    // ── Extra ───────────────────────────────────
    ec("blood_type", "Blood Type", 80.0, T,
        Gen::Pick(BLOOD_TYPES)),
    ec("shirt_size", "Shirt", 60.0, T,
        Gen::Pick(SHIRTS)),
    ec("onboarding", "Onboarding", 110.0, T,
        Gen::Pick(ONBOARDING)),
    ec("clearance", "Clearance", 100.0, T,
        Gen::Pick(CLEARANCES)),
];
    ...
    Some(match col.gen {
        ...
        Gen::Phone => {
            ...
            let mid =
                100 + ((h >> 16) % 900) as u16;
            let end =
                1000 + ((h >> 32) % 9000) as u16;
            format!(
                "+1 ({area:03}) {mid:03}-{end:04}"
            )
        }
        Gen::Date(y_min, y_max) => {
            let h = hash_field(row, salt);
            let range =
                (y_max - y_min + 1) as u64;
        ...
        Gen::Profile => {
            ...
            let l =
                pick(LAST_NAMES, row, salt + 50);
            let n =
                hash_field(row, salt + 99) % 100_000;
            format!(
                "{}-{}-{n:05}",
                f.to_lowercase(),
                l.to_lowercase()
...
pub fn fake_cell(
    ...
    match col_key {
        ...
        "role" => {
            let idx = hash_field(row, 3) as usize
                % ROLES.len();
            Some(ROLES[idx].0.to_owned())
        }
        "dept" => {
            Some(pick(DEPARTMENTS, row, 4).to_owned())
        }
        "salary" => {
            let idx = hash_field(row, 3) as usize
                % ROLES.len();
            ...
            let range = hi - lo;
            let offset =
                hash_field(row, 5) as u32 % (range + 1);
            let salary =
                ((lo + offset) / 1_000) * 1_000;
