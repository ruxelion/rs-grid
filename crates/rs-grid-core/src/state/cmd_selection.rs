    ...
    pub(super) fn cmd_selection(
        ...
        match cmd {
            ...
            GridCommand::SelectRow(row) => {
                let last_col =
                    self.model.columns.len().saturating_sub(1);
                self.selection.anchor =
                    Some(CellCoord { row, col: 0 });
                self.selection.focus =
                    Some(CellCoord { row, col: last_col });
                CommandOutput::None
            }
            ...
            GridCommand::SelectCol(col) => {
                let last_row =
                    self.model.display_row_count().saturating_sub(1);
                self.selection.anchor =
                    Some(CellCoord { row: 0, col });
                self.selection.focus =
                    Some(CellCoord { row: last_row, col });
                CommandOutput::None
            }
            ...
            GridCommand::MoveSelection {
                ...
                if let Some(b) = base {
                    ...
                    let new_row = (b.row as i64 + delta_row)
                        .clamp(
                            0,
                            row_count.saturating_sub(1) as i64,
                        ) as u64;
                    let new_col = (b.col as i64 + delta_col)
                        .clamp(
                            0,
                            col_count.saturating_sub(1) as i64,
                        ) as usize;
                    if extend {
