            ...
            GridCommand::AutoFitColumn {
                ...
                if col_idx < self.model.columns.len() {
                    let old_width =
                        self.model.columns[col_idx].width;
                    let col_key =
                        self.model.columns[col_idx].key.clone();
                    let label =
                        &self.model.columns[col_idx].label;
                    let header_w = label.len() as f64
                        * header_char_width
                        + cell_padding * 2.0;
                    let col_format = self.model.columns[col_idx]
                        .format
                        .clone();
                    let row_count = self
                        .model
