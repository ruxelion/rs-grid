...
description: |
  ...
  "deploy astro with docker", "set up content collections", "add mermaid diagrams to astro",
  "configure astro i18n", "build static blog", or "astro markdown setup". Covers SSG fundamentals,
  Content Collections, Markdown/MDX, partial hydration, islands architecture, and deployment to
  Netlify, Vercel, GitHub Pages, GCP/Firebase, or Docker/nginx.
license: MIT
metadata:
  version: 1.0.0
...
- [Deployment](#deployment) — Firebase URL Config, GitHub Pages, Docker
...
Self-hosted deployment using a multi-stage Docker build. Stage 1 builds the
Astro static site with Node; stage 2 copies `dist/` into an nginx image.
Final image is ~20 MB with no Node runtime.
...
**nginx.conf** — handles clean URLs and long-lived cache for Astro assets:
...
**Astro config** must match nginx `try_files` behavior:
...
| Issue | Solution |
|-------|----------|
| 404 on direct URL | Verify `try_files $uri $uri.html` in nginx.conf |
| Assets not loading | Check `/_astro/` location block |
| Blank page | Confirm `dist/` is non-empty after build stage |
...
## MCP — Astro Docs (real-time)

An official Astro MCP server provides live access to the latest Astro
documentation, avoiding outdated API recommendations.
...
Once configured, use the `mcp__claude_ai_Astro_Docs__search_astro_docs` tool
to query up-to-date Astro docs directly from within a conversation.

## Key Resources

- [Astro Documentation](https://docs.astro.build/)
- [Astro MCP Server](https://docs.astro.build/fr/guides/build-with-ai/)
